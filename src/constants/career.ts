export const CAREERS = [
  {
    ...
    content: [
      ...
      },
      {
        ...
        content:
          'SPA에 필요한 주요 기능이 미리 설정된 보일러플레이트를 개발했습니다. 재사용이 가능한 컴포넌트 라이브러리가 제공되고 다크 모드, 다국어 설정, 반응형 디자인이 지원됩니다. 디자인 토큰으로 컴포넌트를 쉽게 스타일링하고, CSS 변수를 통해 원하는 색상을 토큰으로 추가할 수 있습니다. 보일러플레이트는 NPM에 배포하고 사용 방법을 웹문서로 제공하였는데, 현재까지 누적 다운로드 수는 약 2000회 이상입니다.',
      ...
      },
      {
        ...
        content:
          '개인 블로그를 개발하면서 Next의 주요 개념인 App Directory, App Router, Server Component, Server Action을 경험하였습니다. 특히 역할에 따라 Client 컴포넌트와 Server 컴포넌트를 분리하여 전체 구조를 어떻게 설계할지 많이 고민했습니다. Supabase의 DB, Storage, Authentication을 활용하여 게시물을 직접 관리하고, Vercel로 배포합니다. 평소 개발하면서 얻은 고민과 해결 방법을 기록하고 있습니다.',
    ...
    content: [
      {
        ...
        content:
          'COOV+는 PCR 검사의 모든 과정을 디지털화하여 예약, 접수 문진표 작성, 검사 데이터 입력, 디지털 증명서 발급 기능을 제공하는 서비스입니다. 검사소 관리자 페이지를 담당하여 PCR 검사와 관련된 기능과 검사소 운영에 필요한 기능을 구현하였습니다. 국내외 공항 검사소에서 사용하는 서비스라 외국인 사용자를 위한 다국어 기능을 제공하고 Layout Shift가 발생하지 않게 하였습니다.',
      ...
      },
      {
        ...
        content:
          '팀 자원을 효율적으로 사용하고 개발 속도를 높이기 위해 개발 프로세스를 체계화했습니다. React, TypeScript, Next.js로 기술 스택을 통일하고 렌더링 방식별로 템플릿을 만들었습니다. 빌드 툴을 Webpack에서 Vite로 변경하고, 컴포넌트 라이브러리를 Emotion으로 구현했으며, AWS의 S3와 CloudFront를 사용하는 정적 배포 과정도 자동화했습니다. 꾸준히 프로세스를 개선한 덕분에 팀의 개발 효율과 속도가 높아졌습니다.',
      ...
      },
      {
        ...
        content:
          '중앙 서버가 없는 메신저 BlockChat의 백오피스를 개발했습니다. 앱이 효과적으로 운영될 수 있게 마케터를 위한 대시보드, 개발자를 위한 앱 버전 관리 기능, CS 담당자를 위한 사용권 발주 기능을 구현하였습니다. 사용자 역할에 맞는 권한을 부여하고 모든 이벤트를 로깅하여 나중에 추적할 수 있게 설정하였습니다. 별도로 사용권을 구매하는 페이지도 개발하여 PC와 모바일에서 신용카드로 결제하는 프로세스를 구축했습니다.',
